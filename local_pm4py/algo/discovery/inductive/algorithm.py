...
def apply_bi(logp: Union[EventLog, EventStream, pd.DataFrame], logm: Union[EventLog, EventStream, pd.DataFrame], parameters: Optional[Dict[Any, Any]] = None, variant=DEFAULT_VARIANT_LOG, sup= None, ratio = None, size_par = None, cost_Variant = custom_enum.Cost_Variant.ACTIVITY_FREQUENCY_SCORE, use_gnn = False) -> Tuple[PetriNet, Marking, Marking]:
    """
    Apply the chosen IM algorithm to a log obtaining a Petri net along with an initial and final marking
    ...
    final_marking
        Final marking
    """
    return exec_utils.get_variant(variant).apply(logp,logm, parameters=parameters, sup= sup, ratio = ratio, size_par = size_par, cost_Variant=cost_Variant, use_gnn = use_gnn)
